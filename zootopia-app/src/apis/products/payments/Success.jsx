      ...
      {isConfirmed ? (
        <div className='flex-column align-center confirm-success w-100 max-w-540' style={{display:'flex'}}>
          ...
          <div className='response-section w-100'>
            <div className='flex justify-between'><span className='response-label'>결제 금액</span><span id='amount' className='response-text'>{amount}</span></div>
            <div className='flex justify-between'><span className='response-label'>주문번호</span><span id='orderId' className='response-text'>{orderId}</span></div>
            <div className='flex justify-between'><span className='response-label'>paymentKey</span><span id='paymentKey' className='response-text'>{paymentKey}</span></div>
          </div>
          <div className='w-100 button-group'>
            <div className='flex' style={{gap:'16px'}}>
              <a className='btn w-100' href='https://developers.tosspayments.com/sandbox'>다시 테스트하기</a>
              <a className='btn w-100' href='https://docs.tosspayments.com/guides/v2/payment-widget/integration' target='_blank' rel='noopner noreferer'>결제 연동 문서가기</a>
            </div>
          </div>
        </div>
      ) : (
